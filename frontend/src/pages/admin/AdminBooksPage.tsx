...
import { getAllBooks, createBook, updateBook, deleteBook, Book } from '../../api/book';
...
export const AdminBooksPage: React.FC<AdminBooksPageProps> = ({ currentPage, navigateTo, setIsAdmin }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'coverImage') setFormData({ ...formData, coverImage: files[0] });
    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-4 sm:p-6 lg:p-8">
          ...
          {loading ? (
            <div className="text-center mt-10 text-gray-600">جارٍ تحميل الكتب...</div>
          ) : (
            <Card>
                  ...
                  <TableBody>
                    {books.map((book) => (
                      <TableRow key={book.id}>
                        <TableCell>
                          <ImageWithFallback src={book.coverImage || ''} alt={book.title} className="w-12 h-16 object-cover rounded" />
                        </TableCell>
                        <TableCell>{book.title}</TableCell>
                        <TableCell>{book.author}</TableCell>
                        <TableCell>{book.price} ر.س</TableCell>
                        <TableCell>{categories.find((c) => c.id === book.categoryId)?.name || '-'}</TableCell>
      ...
      {/* Modal */}
{modalOpen && (
  <div className="fixed inset-0 flex items-start justify-center bg-black bg-opacity-50 z-50 p-4 pt-16">
    <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl border border-gray-200 overflow-y-auto max-h-[80vh]">
      ...
      <div className="flex justify-between items-center bg-purple-600 text-white px-4 py-2 rounded-t-xl">
        <h2 className="text-lg font-bold">{selectedBook ? 'تعديل الكتاب' : 'إضافة كتاب جديد'}</h2>
      ...
      <form onSubmit={handleSubmit} className="p-4 grid gap-4">
        {errorMessage && <div className="text-sm text-red-700 bg-red-50 border border-red-100 p-2 rounded">{errorMessage}</div>}
        {successMessage && <div className="text-sm text-green-700 bg-green-50 border border-green-100 p-2 rounded">{successMessage}</div>}
        ...
        <div className="flex flex-col gap-1">
          <Label>التصنيف</Label>
          <select name="categoryId" value={formData.categoryId} onChange={handleChange} className="border rounded px-2 py-1">
        ...
        <div className="flex justify-end gap-2 mt-2">
          ...
          <Button type="submit" className="bg-purple-600 hover:bg-purple-700 text-white" disabled={isSaving}>
            {isSaving ? 'جاري الحفظ...' : selectedBook ? 'تحديث' : 'إنشاء'}
          </Button>
        </div>
      </form>
    </div>
